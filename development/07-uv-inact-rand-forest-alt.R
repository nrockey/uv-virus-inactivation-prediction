...
# subset.
file_model = sprintf('results/rf/rf-opt-%s.RData', data_subset)
...
file_mse = sprintf('results/rf/rf-summ-loocv-%s.RData', data_subset)
...
for ( class in names(summ_rf_all) ) {
  ...
  # Generate an R Data file name for mse training info.
  file_summ = sprintf('csv-outputs/rf-summ-kval-%s%s.csv',
                      class,
