...
# subset.
file_model = sprintf('results/xgb/xgb-opt-%s.RData', data_subset)
...
file_mse = sprintf('results/xgb/xgb-summ-loocv-%s.RData', data_subset)
...
# file_round_error = sprintf('csv-outputs/xgb-rounds-error-%s.csv', data_subset)
...
for ( class in names(summ_xgb_all) ) {
  ...
  # Generate an R Data file name for mse training info.
  file_summ = sprintf('csv-outputs/xgb-summ-kval-%s%s.csv',
                      class,
