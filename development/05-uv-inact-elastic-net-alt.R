...
# subset.
file_model = sprintf('results/elnt/elnt-opt-%s.RData', data_subset)
...
file_mse = sprintf('results/elnt/elnt-summ-loocv-%s.RData', data_subset)
...
for ( class in names(summ_elnt_all) ) {
  ...
  # Generate an R Data file name for mse training info.
  file_summ = sprintf('csv-outputs/elnt-summ-kval-%s%s.csv',
                      class,
