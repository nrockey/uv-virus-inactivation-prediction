...
# the model or removed. This only needs to be done for the 'all' and 'dsdna'
# inputs. For (+) ssRNA viruses, these should always be set to 'FALSE' because
# the repair and host variables are not part of the model.
...
# data subset.
file_model = sprintf('development/development-results/mlr/mlr-opt-rep-%s-host-%s-%s.RData',
                     include_repair,
                     include_host,
                     data_subset)
...
file_mse = sprintf('development/development-results/mlr/mlr-summ-loocv-rep-%s-host-%s-%s.RData',
                   include_repair,
                   include_host,
                   data_subset
)
...
for ( class in names(summ_mlr_all) ) {
  ...
  # Generate an R Data file name for mse training info.
  file_summ = sprintf('development/csv-outputs/mlr-summ-kval-%s%s.csv',
                      class,
